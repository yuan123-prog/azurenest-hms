...
if ($search !== '') {
    ...
    $where = "WHERE room_number LIKE '%$search_sql%' OR status LIKE '%$search_sql%'";
    foreach ($type_options as $id => $name) {
        if (stripos($name, $search) !== false) {
            $where = $where ? $where . " OR type_id=$id" : "WHERE type_id=$id";
        }
    }
}
$rooms = $conn->query("SELECT * FROM Rooms $where");
